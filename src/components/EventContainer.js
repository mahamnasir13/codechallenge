...
class EventContainer extends Component {
  handleSearch = (event) => {
    if (event.target.value === "See all") {
      ...
      let events = this.props.events.filter((show) => {
        return (
          show.venue.city ===
          event.target.value.slice(0, event.target.value.length - 6)
        );
      });
  ...
  render() {
    ...
    let optionItems = events.map((event) => (
      <option key={event.venue.city}>
        {event.venue.city} {event.datetime.slice(5, 10)}
      </option>
    ));
          ...
          <div>
            ...
            <div className="d-flex flex-wrap">
              {this.props.events.map((event) => (
                <EventCard
                  event={event}
                  addToMyEvents={this.props.addToMyEvents}
                  myEventsClicked={this.props.myEventsClicked}
                />
              ))}
            </div>
